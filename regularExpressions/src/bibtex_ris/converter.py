...
class BibtexRisConverter:
    ...
    def parse_bibtex(self, content):
        ...
        entries = []
        entry_pattern = re.compile(r'@(\w+)\s*\{\s*([^,]+),', re.IGNORECASE | re.DOTALL)

        for match in entry_pattern.finditer(content):
            entry_type = match.group(1).lower()
            entry_id = match.group(2).strip()

            # Cierre de la entrada contando llaves
    ...
    def _parse_fields(self, fields_text):
        # Acepta valores entre llaves o comillas y comandos LaTeX anidados
        field_pattern = (
            r'(\w+)\s*=\s*'
            r'(?:"([^"]*)"|\{((?:[^{}]|\{\\[a-zA-Z]+\{[^{}]*\}\}|\{\\[^{}]{1,2}\})*)\})'
        )
    ...
    def parse_ris(self, content):
        ...
        # Cada bloque termina con la etiqueta ER
        for block in re.split(r'ER\s*-', content):
            if not block.strip():
                continue

            entry = {}

            # La etiqueta puede llevar digitos como T2
            line_pattern = r'(\w+)\s*-\s*(.+?)(?=\n[A-Z0-9]+\s*-|\Z)'

            for match in re.finditer(line_pattern, block, re.DOTALL):
                tag = match.group(1).strip()
                value = match.group(2).strip()

                # Etiquetas repetidas se agrupan en una lista
                if tag in entry:
    ...
    def ris_entry_to_bibtex(self, entry):
        ...
        for tag, value in entry.items():
            if tag in handled or tag in ('TY', 'ID', 'ER', 'PY', 'DA', 'SP', 'EP', 'AU', 'ED', 'KW', 'SN', 'JO', 'T2', 'BT'):
                continue
            bibtex_field = self.RIS_TO_BIBTEX.get(tag)
            if not bibtex_field:
                continue
